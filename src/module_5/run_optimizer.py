...
LLM_PROMPT_PATH = Path(__file__).parent / "prompts/llm_prompt.txt"
LLM_OUTPUT_PATH = OUTPUTS_DIR / "llm_output.txt"
JUDGE_OUTPUT_PATH = OUTPUTS_DIR / "judge_output.txt"
OPTIMIZED_PROMPT_PATH = OUTPUTS_DIR / "optimized_prompt.txt"
...
def main():
    try:
        ...
        variables = {
            "previous_final_answer_prompt": load_file(LLM_PROMPT_PATH),
            "judge_response": load_file(JUDGE_OUTPUT_PATH)
        }
        prompt = fill_prompt(template, variables)
        answer = call_openrouter_llm(prompt, API_KEY, model=MODEL)
        write_output(answer, OPTIMIZED_PROMPT_PATH)
